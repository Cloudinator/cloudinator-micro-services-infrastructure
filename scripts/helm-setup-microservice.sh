...
main() {
    ...
    yq eval ".ingress.hosts[0].paths[0].path = \"/\"" -i values.yaml
    yq eval ".ingress.hosts[0].paths[0].pathType = \"Prefix\"" -i values.yaml
    yq eval ".ingress.tls[0].secretName = \"$CHART_NAME-tls\"" -i values.yaml
    yq eval ".ingress.tls[0].hosts[0] = \"$HOST\"" -i values.yaml
